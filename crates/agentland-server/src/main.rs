//! Agentland Platform — commercial AI agent governance server.
//!
//! Embeds the Agentland proxy engine and adds:
...
use agentland_proxy::{api as oss_api, events, policy::PolicyHook, proxy};
use agentland_common::config::PlatformConfig;
use agentland_common::license::{self, LicenseTier};
use agentland_identity::{ca::CertificateAuthority, mtls::MtlsConfig};
use agentland_policy::budget::{BudgetLimit, BudgetTracker};
use agentland_policy::engine::PolicyEngine;
use agentland_policy::hook::AgentlandPolicyHook;
use tracing_subscriber::EnvFilter;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    // ── Logging ───────────────────────────────────────────────────────────────
    // Priority: RUST_LOG (fine-grained) > AGENTLAND_LOG_LEVEL > Agentland_LOG_LEVEL > "info"
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_env("RUST_LOG")
                .or_else(|_| EnvFilter::try_from_env("AGENTLAND_LOG_LEVEL"))
    ...
    tracing::info!(
        ...
        "Agentland Platform starting"
    );

    // ── License validation ──────────────────────────────────────────────────
    let license_key = std::env::var("AGENTLAND_LICENSE_KEY").ok();
    ...
    let (mtls_config, platform_ca) = if license_info.a2a_identity_enabled {
        let org_name = license_info.org_id.as_deref().unwrap_or("agentland");
    ...
    // ── Tenant registry ──────────────────────────────────────────────────────
    let tenant_registry = Arc::new(agentland_common::tenant_registry::TenantRegistry::new());
    tracing::info!(
        tenants = tenant_registry.count(),
        "tenant registry initialized"
    );

    // ── Agentland configuration ──────────────────────────────────────────
    ...
    let agentland_config_path =
        std::env::var("AGENTLAND_CONFIG").unwrap_or_else(|_| "config/agentland.default.toml".to_string());
    let platform_cfg = PlatformConfig::load(&agentland_config_path);

    // ── Database pool ───────────────────────────────────────────────────────
    let pool = match agentland_store::connect(&config.database).await {
        Ok(p) => {
    ...
    for (agent_id, max_tokens) in &platform_cfg.platform.agent_token_limits {
        budget.set_agent_limit(
            agent_id.clone(),
            agentland_policy::budget::BudgetLimit {
    ...
    let policy_hook: Arc<dyn PolicyHook> = Arc::new(
        AgentlandPolicyHook::new(Arc::clone(&policy_engine), pii_enabled)
