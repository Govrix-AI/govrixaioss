...
//!
//! Extends the Agentland management API with policy and tenant management routes.
...
use agentland_common::license::LicenseTier;
use agentland_common::tenant_registry::TenantRegistry;
...
pub struct PlatformState {
    ...
    pub ca: Option<Arc<agentland_identity::ca::CertificateAuthority>>,
...
async fn issue_cert(
    ...
    let cert = agentland_identity::certs::issue_agent_cert(ca, &req.agent_id)
...
) -> Result<Json<agentland_policy::compliance::ComplianceReport>, (StatusCode, String)> {
    use agentland_policy::compliance::ComplianceReport;
...
async fn demo_session_recording() -> Json<serde_json::Value> {
    use agentland_policy::session::{SessionEvent, SessionRecording};
...
///
/// If `AGENTLAND_API_KEY` is set, all requests must include
...
async fn require_api_key(req: Request<Body>, next: Next) -> Response {
    let expected = std::env::var("AGENTLAND_API_KEY").unwrap_or_default();
    ...
    match auth_header {
        ...
        None => (
            ...
            Json(serde_json::json!({
                ...
                "hint": "use Authorization: Bearer <AGENTLAND_API_KEY>",
            })),
        )
            .into_response(),
    }
...
pub fn platform_router(state: Arc<PlatformState>) -> Router {
    // Protected routes require a valid API key when AGENTLAND_API_KEY is set.
...
mod tests {
    ...
    fn make_state() -> Arc<PlatformState> {
        Arc::new(PlatformState {
            license_tier: agentland_common::license::LicenseTier::Enterprise,
            max_agents: 10,
            policy_enabled: true,
