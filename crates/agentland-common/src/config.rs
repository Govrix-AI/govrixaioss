...
//!
//! All environment variables use the `AGENTLAND_` prefix.
//! Env vars take precedence over values in the TOML file.

use serde::{Deserialize, Serialize};
use std::path::Path;

/// Top-level Agentland configuration.
...
pub struct DatabaseConfig {
    /// PostgreSQL connection URL. Use `AGENTLAND_DATABASE_URL` env var in production.
    pub url: String,
...
impl Default for DatabaseConfig {
    fn default() -> Self {
        Self {
            url: "postgres://agentland:agentland@localhost:5432/agentland".to_string(),
            max_connections: 20,
            min_connections: 2,
...
impl Config {
    ...
    ///
    /// Environment variables (all prefixed with `AGENTLAND_`):
    /// - `AGENTLAND_DATABASE_URL`        → database.url
    /// - `AGENTLAND_PROXY_PORT`          → proxy.port
    /// - `AGENTLAND_API_PORT`            → api.port
    /// - `AGENTLAND_LOG_LEVEL`           → telemetry.log_level
    /// - `AGENTLAND_PROXY_FAIL_OPEN`     → proxy.fail_open
    ...
    /// Apply `AGENTLAND_*` environment variable overrides.
    fn apply_env_overrides(&mut self) {
        if let Ok(url) = std::env::var("AGENTLAND_DATABASE_URL") {
            self.database.url = url;
        }
        if let Ok(port) = std::env::var("AGENTLAND_PROXY_PORT") {
            if let Ok(p) = port.parse() {
                self.proxy.port = p;
            }
        }
        if let Ok(port) = std::env::var("AGENTLAND_API_PORT") {
            if let Ok(p) = port.parse() {
                self.api.port = p;
            }
        }
        if let Ok(level) = std::env::var("AGENTLAND_LOG_LEVEL") {
            self.telemetry.log_level = level;
        }
        if let Ok(val) = std::env::var("AGENTLAND_PROXY_FAIL_OPEN") {
            self.proxy.fail_open = val.to_lowercase() != "false";
        }
        if let Ok(url) = std::env::var("AGENTLAND_PROXY__UPSTREAM_OPENAI") {
            self.proxy.upstream_openai = url;
        }
        if let Ok(url) = std::env::var("AGENTLAND_PROXY__UPSTREAM_ANTHROPIC") {
