...
pub struct SseAccumulator {
    ...
    pub anthropic_events: Vec<(
        agentland_common::protocols::anthropic::AnthropicSseEvent,
...
impl SseAccumulator {
    ...
    pub fn process_anthropic_chunk(&mut self, chunk: &Bytes) {
        let events = agentland_common::protocols::anthropic::parse_sse_events(chunk);
        for (event, data) in events {
            if event == agentland_common::protocols::anthropic::AnthropicSseEvent::MessageStop {
                self.is_done = true;
            }
