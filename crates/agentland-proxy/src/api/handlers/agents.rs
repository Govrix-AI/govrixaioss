...
use agentland_store::AgentFilter;
...
pub async fn list_agents(
    ...
    match agentland_store::agents::list_agents(&state.pool, &filter).await {
        Ok(agents) => {
...
pub async fn get_agent(
    ...
    match agentland_store::agents::get_agent(&state.pool, &id).await {
...
pub async fn update_agent(
    ...
    let result = agentland_store::agents::update_agent_metadata(
        &state.pool,
        &id,
        body.name.as_deref(),
...
pub async fn retire_agent(
    ...
    match agentland_store::agents::retire_agent(&state.pool, &id).await {
        Ok(_) => (
            StatusCode::OK,
            Json(json!({
...
pub async fn get_agent_events(
    ...
    match agentland_store::events::get_events_for_agent(&state.pool, &id, limit, None).await {
        Ok(events) => {
...
pub async fn get_agent_runs(
    ...
    match agentland_store::events::get_agent_runs(&state.pool, &id, limit).await {
        Ok(runs) => {
            let total = runs.len();
            (
                StatusCode::OK,
...
pub async fn get_agent_violations(
    ...
    match agentland_store::events::get_agent_violations(&state.pool, &id, limit).await {
        Ok(violations) => {
