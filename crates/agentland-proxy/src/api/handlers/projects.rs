...
pub async fn list_projects(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    match agentland_store::projects::list_projects(&state.pool).await {
        Ok(projects) => {
...
pub async fn create_project(
    ...
    match agentland_store::projects::create_project(
        &state.pool,
        &body.name,
...
pub async fn get_project(
    ...
    match agentland_store::projects::get_project(&state.pool, uuid).await {
...
pub async fn update_project(
    ...
    match agentland_store::projects::update_project(
        &state.pool,
        uuid,
        body.name.as_deref(),
        body.description.as_deref(),
    ...
    .await
    {
        Ok(true) => match agentland_store::projects::get_project(&state.pool, uuid).await {
            Ok(Some(project)) => (StatusCode::OK, Json(json!({ "data": project }))),
            _ => (
                StatusCode::OK,
...
pub async fn delete_project(
    ...
    match agentland_store::projects::delete_project(&state.pool, uuid).await {
        Ok(Ok(true)) => (
            StatusCode::OK,
...
pub async fn list_project_agents(
    ...
    match agentland_store::projects::list_project_agents(&state.pool, uuid).await {
        Ok(agents) => {
