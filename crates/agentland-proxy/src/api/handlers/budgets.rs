...
pub async fn get_agent_budget(
    ...
    let config = agentland_store::budget::get_budget_config(&state.pool, &id).await;
    let usage = agentland_store::budget::get_budget_today(&state.pool, &id).await;
...
pub async fn set_agent_budget(
    ...
    match agentland_store::budget::upsert_budget_config(
        &state.pool,
        &id,
...
pub async fn delete_agent_budget(
    ...
    match agentland_store::budget::delete_budget_config(&state.pool, &id).await {
        Ok(true) => (
            StatusCode::OK,
...
pub async fn budget_overview(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    match agentland_store::budget::get_budget_overview(&state.pool).await {
        Ok(rows) => {
            let total = rows.len();
            (
                StatusCode::OK,
