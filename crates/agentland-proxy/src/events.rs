//! Event channel and background writer for Agentland proxy.
//!
//! Architecture:
...
use agentland_common::models::agent::{Agent, AgentType};
...
pub async fn run_background_writer(
    ...
    pool: Option<agentland_store::StorePool>,
    metrics: Arc<Metrics>,
) {
...
async fn flush_batch(
    ...
    match pool {
        Some(p) => {
            match agentland_store::events::insert_events_batch(p, batch).await {
                Ok(count) => {
...
async fn upsert_agents_from_batch(
    pool: &agentland_store::StorePool,
    batch: &[AgentEvent],
    metrics: &Arc<Metrics>,
    seen_agents: &mut std::collections::HashSet<String>,
) {
    ...
    for (agent_id, accum) in &agents {
        ...
        if let Err(e) = agentland_store::agents::upsert_agent(pool, &agent).await {
            tracing::warn!(
                error = %e,
                agent_id = %agent_id,
...
mod tests {
    ...
    fn event_channel_try_send_non_blocking() {
        ...
        rt.block_on(async {
            ...
            let event = agentland_common::models::event::AgentEvent::new(
                "agent-1",
                uuid::Uuid::now_v7(),
                agentland_common::models::event::EventDirection::Outbound,
                "POST",
                "https://api.openai.com/v1/chat/completions",
                agentland_common::models::event::Provider::OpenAI,
                "genesis",
                "audit:none",
            );
