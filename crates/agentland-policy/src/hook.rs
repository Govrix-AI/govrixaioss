//! Bridge between the OSS [`PolicyHook`] trait and the Agentland [`PolicyEngine`].
//!
//! [`AgentlandPolicyHook`] wraps the platform's policy engine so it can be
//! injected into the OSS proxy interceptor, providing compliance tagging
//! and request blocking via the Agentland rule set.
//!
...
use agentland_proxy::policy::PolicyHook;

use crate::budget::{BudgetResult, BudgetTracker};
use crate::engine::{PolicyDecision, PolicyEngine};
use crate::pii;

/// Bridges the OSS `PolicyHook` trait to the Agentland `PolicyEngine`.
///
...
///
/// When a [`BudgetTracker`] is attached via [`AgentlandPolicyHook::with_budget`],
...
pub struct AgentlandPolicyHook {
    engine: Arc<RwLock<PolicyEngine>>,
    pii_enabled: bool,
    budget: Option<Arc<BudgetTracker>>,
}

impl AgentlandPolicyHook {
...
impl PolicyHook for AgentlandPolicyHook {
...
mod tests {
    ...
    use agentland_common::models::event::{EventDirection, Provider};
    use uuid::Uuid;
    ...
    fn empty_engine_returns_pass_all() {
        let hook = AgentlandPolicyHook::new(make_engine(), false);
    ...
    fn blocking_rule_returns_block_tag() {
        ...
        let hook = AgentlandPolicyHook::new(engine, false);
        let event = make_event();
    ...
    fn alert_rule_returns_warn_tag_and_allows_request() {
        ...
        let hook = AgentlandPolicyHook::new(engine, true);
        let event = make_event();
    ...
    fn pii_detected_in_tags_elevates_to_warn() {
        let hook = AgentlandPolicyHook::new(make_engine(), true);
    ...
    fn budget_within_limit_passes() {
        ...
        let hook = AgentlandPolicyHook::new(make_engine(), false).with_budget(tracker);
        let event = make_event();
    ...
    fn engine_block_takes_priority_over_budget() {
        ...
        let hook = AgentlandPolicyHook::new(engine, false).with_budget(tracker);
        let mut event = make_event();
        event.total_tokens = Some(1);
